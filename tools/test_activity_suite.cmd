...
set "PROJECT=%~dp0.."
set "OUT_DIR=%TEMP%\kitsu868-activity-suite-test"
...
if not exist "%GNUXX%" (
  echo TEST_BLOCKED activity_suite: PlatformIO GNU compiler not found
  exit /b 2
)
"%GNUXX%" -std=gnu++11 -Wall -Wextra -Werror ^
  -I"%PROJECT%\src" -fsyntax-only ^
  "%PROJECT%\src\activity_suite.cpp" ^
  "%PROJECT%\tools\test_activity_suite.cpp"
if errorlevel 1 exit /b %ERRORLEVEL%
...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED activity_suite: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\src\activity_suite.cpp" ^
  "%PROJECT%\tools\test_activity_suite.cpp" ^
  /Fe:"%OUT_DIR%\activity_suite_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\activity_suite_test.exe"
