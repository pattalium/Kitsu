...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_endpoint_dispatcher: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-endpoint-dispatcher-test"
...
cl /nologo /c /std:c++17 /EHsc /W0 /DHOST_BUILD ^
  ...
  "%MESHCORE%\Packet.cpp" "%MESHCORE%\Dispatcher.cpp"
if errorlevel 1 (
  popd
  exit /b 1
)
cl /nologo /std:c++17 /EHsc /W4 /WX /wd4100 /wd4244 /wd4245 /DHOST_BUILD ^
  /FI"%HOST_COMPAT%\crypto_compat.h" ^
  /I"%HOST_COMPAT%" /I"%CRYPTO%" /I"%CRYPTO%\utility" /I"%MESHCORE%" ^
  "%PROJECT%\tools\test_kitsu_endpoint_dispatcher.cpp" ^
  Crypto.obj Hash.obj SHA256.obj Packet.obj Dispatcher.obj ^
  /Fe:"%OUT_DIR%\kitsu_endpoint_dispatcher_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_endpoint_dispatcher_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
popd
echo Kitsu endpoint Dispatcher 32-frame burst test passed.
