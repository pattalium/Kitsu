...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_party_hotspot: Visual C++ build tools not found
  exit /b 2
)
...
set "OUT_DIR=%TEMP%\kitsu868-party-hotspot-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\tools\test_kitsu_party_hotspot.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_party_hotspot_test.exe"
if errorlevel 1 (
...
"%OUT_DIR%\kitsu_party_hotspot_test.exe"
set "RESULT=%ERRORLEVEL%"
popd
exit /b %RESULT%
