...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED signal_encounter: Visual C++ build tools not found
  exit /b 2
)
...
set "OUT_DIR=%TEMP%\kitsu868-signal-encounter-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\tools\test_signal_encounter.cpp" ^
  /Fe:"%OUT_DIR%\signal_encounter_test.exe"
if errorlevel 1 (
...
"%OUT_DIR%\signal_encounter_test.exe"
set "RESULT=%ERRORLEVEL%"
popd
exit /b %RESULT%
