...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_enrollment: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-enrollment-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\tools\test_kitsu_enrollment.cpp" ^
  bcrypt.lib ^
  /Fe:"%OUT_DIR%\kitsu_enrollment_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_enrollment_test.exe"
...
popd
echo Kitsu enrollment tests passed.
