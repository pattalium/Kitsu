...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_ble_ota: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-ble-ota-test"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
if errorlevel 1 exit /b %ERRORLEVEL%

pushd "%OUT_DIR%"
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  "%PROJECT%\src\kitsu_ble_ota.cpp" ^
  "%PROJECT%\tools\test_kitsu_ble_ota.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_ble_ota_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_ble_ota_test.exe"
...
popd
echo Kitsu BLE OTA tests passed.
