...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_ble_session: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-ble-session-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\src\kitsu_ble_session.cpp" ^
  "%PROJECT%\tools\test_kitsu_ble_session.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_ble_session_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_ble_session_test.exe"
...
popd
echo Kitsu BLE session tests passed.
