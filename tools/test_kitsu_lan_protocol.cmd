...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_lan_protocol: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-lan-protocol-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\tools\test_kitsu_lan_protocol.cpp" ^
  bcrypt.lib ^
  /Fe:"%OUT_DIR%\kitsu_lan_protocol_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_lan_protocol_test.exe"
...
popd
echo Kitsu LAN protocol tests passed.
