...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_transport_scope: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-transport-scope-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX /wd4100 /wd4244 /wd4245 /DHOST_BUILD ^
  ...
  "%PROJECT%\tools\test_kitsu_transport_scope.cpp" ^
  Crypto.obj Hash.obj SHA256.obj ^
  /Fe:"%OUT_DIR%\kitsu_transport_scope_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_transport_scope_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
popd
echo Kitsu transport scope tests passed.
