...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_channel_repeat_tracker: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-channel-repeat-tracker-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX /wd4100 /wd4244 /wd4245 /DHOST_BUILD ^
  ...
  "%PROJECT%\tools\test_kitsu_channel_repeat_tracker.cpp" ^
  Crypto.obj Hash.obj SHA256.obj Packet.obj ^
  /Fe:"%OUT_DIR%\kitsu_channel_repeat_tracker_test.exe"
if errorlevel 1 (
  set "RESULT=%ERRORLEVEL%"
  popd
  exit /b %RESULT%
)
"%OUT_DIR%\kitsu_channel_repeat_tracker_test.exe"
if errorlevel 1 (
  set "RESULT=%ERRORLEVEL%"
  popd
  exit /b %RESULT%
...
popd
echo Kitsu channel repeat tracker tests passed.
