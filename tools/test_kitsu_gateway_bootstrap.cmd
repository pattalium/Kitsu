...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_gateway_bootstrap: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-gateway-bootstrap-test"
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\src\kitsu_gateway_bootstrap.cpp" ^
  "%PROJECT%\tools\test_kitsu_gateway_bootstrap.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_gateway_bootstrap_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_gateway_bootstrap_test.exe"
...
popd
echo Kitsu gateway bootstrap tests passed.
