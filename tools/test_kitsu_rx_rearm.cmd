...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_rx_rearm: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%

set "OUT_DIR=%TEMP%\kitsu868-rx-rearm-test"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
if errorlevel 1 exit /b %ERRORLEVEL%

pushd "%OUT_DIR%"
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  "%PROJECT%\tools\test_kitsu_rx_rearm.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_rx_rearm_test.exe"
if errorlevel 1 (
  set "RESULT=%ERRORLEVEL%"
  popd
  exit /b %RESULT%
)
"%OUT_DIR%\kitsu_rx_rearm_test.exe"
if errorlevel 1 (
  set "RESULT=%ERRORLEVEL%"
  popd
  exit /b %RESULT%
...
popd
echo Kitsu SX1262 RX rearm policy tests passed.
