...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_chat_contract: Visual C++ build tools not found
  exit /b 2
)
...
set "OUT_DIR=%TEMP%\kitsu868-chat-contract-test"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
if errorlevel 1 exit /b %ERRORLEVEL%

pushd "%OUT_DIR%"
cl /nologo /std:c++17 /utf-8 /EHsc /W4 /WX ^
  "%PROJECT%\src\kitsu_chat_contract.cpp" ^
  "%PROJECT%\tools\test_kitsu_chat_contract.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_chat_contract_test.exe"
if errorlevel 1 (
...
"%OUT_DIR%\kitsu_chat_contract_test.exe"
set "RESULT=%ERRORLEVEL%"
popd
exit /b %RESULT%
