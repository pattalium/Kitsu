...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED encounter_protocol: Visual C++ build tools not found
  exit /b 2
)
...
set "OUT_DIR=%TEMP%\kitsu868-encounter-test"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
if errorlevel 1 exit /b %ERRORLEVEL%

pushd "%OUT_DIR%"
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  "%PROJECT%\src\encounter_protocol.cpp" ^
  "%PROJECT%\tools\test_encounter_protocol.cpp" ^
  /Fe:"%OUT_DIR%\encounter_protocol_test.exe"
if errorlevel 1 (
...
"%OUT_DIR%\encounter_protocol_test.exe"
set "RESULT=%ERRORLEVEL%"
popd
exit /b %RESULT%
