...
set "PROJECT=%~dp0.."
set "OUT_DIR=%TEMP%\kitsu868-focus-session-test"
...
if not exist "%GNUXX%" (
  echo TEST_BLOCKED kitsu_focus_session: PlatformIO GNU compiler not found
  exit /b 2
)
"%GNUXX%" -std=gnu++11 -Wall -Wextra -Werror ^
  -I"%PROJECT%\src" -fsyntax-only ^
  "%PROJECT%\src\kitsu_focus_session.cpp" ^
  "%PROJECT%\tools\test_kitsu_focus_session.cpp"
if errorlevel 1 exit /b %ERRORLEVEL%
...
if not exist "%VCVARS%" (
  echo TEST_BLOCKED kitsu_focus_session: Visual C++ build tools not found
  exit /b 2
)
call "%VCVARS%" >nul
if errorlevel 1 exit /b %ERRORLEVEL%
...
cl /nologo /std:c++17 /EHsc /W4 /WX ^
  ...
  "%PROJECT%\src\kitsu_focus_session.cpp" ^
  "%PROJECT%\tools\test_kitsu_focus_session.cpp" ^
  /Fe:"%OUT_DIR%\kitsu_focus_session_test.exe"
if errorlevel 1 (
  popd
  exit /b 1
)
"%OUT_DIR%\kitsu_focus_session_test.exe"
