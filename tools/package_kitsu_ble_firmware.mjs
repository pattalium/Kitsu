...
export const PARTITION_BYTES = 0x330000;
...
function validateIdentity(releaseId, firmwareVersion) {
  ...
  if (Buffer.byteLength(firmwareVersion, "ascii") > 32 ||
      !semverPattern.test(firmwareVersion)) {
...
function validateImage(image) {
  validateEsp32S3Application(image);
  return sha256(image);
}
...
export function prepareManifest({ image, releaseId, firmwareVersion }) {
  const imageSha256 = validateImage(image);
...
function cli() {
  ...
  if (command === "prepare") {
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      image_bytes: imagePath.stat.size,
      image_sha256: sha256(readFileSync(imagePath.absolute)),
    })}\n`);
    return;
  }
