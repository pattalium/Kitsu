...
protected:
  ...
  // Kitsu's volatile TX lock must distinguish work still owned by the queue
  // from the one packet whose asynchronous radio send has already started.
  // Expose that pointer to subclasses without allowing them to mutate it.
  const Packet* currentOutboundPacket() const { return outbound; }
